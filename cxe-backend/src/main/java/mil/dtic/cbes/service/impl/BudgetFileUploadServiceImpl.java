...
//		return false;
	...
	public void deleteFile(Integer id) {
		...
		File file = new File(budgetFile.get().getUrl());
		if (file != null) {
			file.delete();
		}
		...
		if (maxNumberOfFiles != null) {
			...
			if (files.size() > maxNumberOfFiles) {
				...
				for (UploadedBudgetFile uploadedFile : filesToRemove) {
					File file = new File(uploadedFile.getUrl());
					file.delete();
	...
	private void checkFilesExist(List<UploadedBudgetFile> uploadedBudgetFiles) {
		for (UploadedBudgetFile uploadedBudgetFile : uploadedBudgetFiles) {
			File file = new File(uploadedBudgetFile.getUrl());
